...
def comment_create(request, article_pk):
    ...
    # 유효성 검사를 통과하는지를 확인 (comment_form에는 애초에 content 필드만 있다)

    if comment_form.is_valid():
        # comment가 가지고 있어야 할 article (참조 대상) 정보가 누락
        # CommentForm의 fields에 content만 정의해 놨으므로
        # article 에 대한 정보는 사용자가 입력하지 않도록 만들었으므로...
        # 하지만, table에 comment를 생성하려면?
        # article FK 필요하다. -> 넣어줘야됨
        comment = comment_form.save(commit=False) # db에 반영이 되는데 저장만해
